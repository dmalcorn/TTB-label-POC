...
- ``PRAGMA journal_mode = WAL`` persists on the database file — set once in
  :func:`init_db`. WAL lets the Epic-2 pipeline's concurrent writers (OCR job +
  analysis job) proceed without ``SQLITE_BUSY``.
...
def get_connection(db_path: str | Path) -> sqlite3.Connection:
    ...
    snake_case in the schema, so no translation is needed).
    """
