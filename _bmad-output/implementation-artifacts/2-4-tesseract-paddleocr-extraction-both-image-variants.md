...
### Railway Deploy Blocker → FIXED (found 2026-06-13 via the Railway deploy after the push)

The image **built** fine on Railway but the deploy failed the healthcheck — `/healthz` never came up because the app crashed at startup:

- [x] [Deploy][Blocker] **`init_db` crashed on the persistent Volume DB** — `sqlite3.OperationalError: no such column: image_variant` ([connection.py](app/db/connection.py) `executescript` → [main.py:84](app/main.py#L84) lifespan → app exits → healthcheck fails after 11 retries / 5 min). The Volume DB was created by an earlier story; `CREATE TABLE IF NOT EXISTS ocr_results` skips the existing table so `image_variant` was never added, and `CREATE INDEX … idx_ocr_results_variant` then referenced a missing column. This project has **no migration framework**, so adding a column to an existing table on the Volume had no upgrade path. **Fix (Diane's call — guarded ALTER):** `init_db` now runs `_apply_added_columns` before replaying the schema — an idempotent `ADD COLUMN if missing` over a ledger (`_ADDED_COLUMNS`) of post-creation additions. Also backfills **Story 2.3's** `label_images` columns (`enhanced_path`/`binarized_path`/`preprocess_log`/`preprocess_ms`/`preprocessed_at`), which the frozen Volume DB was *also* missing (2.3 didn't crash only because no DDL referenced them). Tests: `test_init_db_backfills_columns_on_legacy_volume_db`, `test_init_db_backfill_is_idempotent`. Host: 158 passed, 1 skipped, ruff clean. Append to `_ADDED_COLUMNS` whenever a future story adds a column to a pre-existing table (recurs throughout Epic 3).
